...
class 生成器类:
    ...
    def _生成相似度语料库(
        ...
        for 语料类别 in 语料库.keys():
            # 计算相似度、排序、并裁剪语料
            相似度语料库[语料类别] = self._生成类别相似度语料库(主题词, 语料库[语料类别], 计算相似度并排序=True)[:10]
        return 相似度语料库
    ...
    def 生成素材(self, 主题谓语: str = "", 主题宾语: str = "", 语料类别: str = "事例", 素材数量: int = 10):
        if 语料类别 not in self.语料库:
            return 作文类()
        # 第一步：生成素材列表
        主题词 = f"{主题谓语}{主题宾语}"
        素材与相似度列表: list[tuple[str, float]] = self._生成类别相似度语料库(
            主题词, self.语料库[语料类别], 计算相似度并排序=True
        )[:素材数量]
        素材列表: list[str] = [素材与相似度[0] for 素材与相似度 in 素材与相似度列表]
        # 第二步：替换主题词
        素材列表 = self._替换主题词(素材列表, 主题谓语, 主题宾语)
        # 第三步: 生成统计信息
        段数 = len(素材列表)
        字数 = sum(len(段落) for 段落 in 素材列表)
        return 作文类(文章=素材列表, 段数=段数, 字数=字数, 谓语=主题谓语, 宾语=主题宾语)
